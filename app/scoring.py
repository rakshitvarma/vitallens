...
def compute_score(
    ...
    start = _coerce_date(start_date) or (today - timedelta(days=7))
    end = _coerce_date(end_date) or today
    period_days = max((end - start).days + 1, 1)
    activity_target = t["activity_min_per_week"] * period_days / 7
    consistency_target = min(period_days, 7)

    meals = [m for m in meals if _in_range(m.get("date", m.get("created_at")), start, end)]
    activities = [a for a in activities if _in_range(a.get("date", a.get("created_at")), start, end)]

    days_logged = len({str(m.get("date", ""))[:10] for m in meals})
    days = max(days_logged, 1)

    total = lambda key: sum(float(m.get(key) or 0) for m in meals)  # noqa: E731
    avg_cal = total("calories") / days
    avg_sugar = total("sugar_g") / days
    avg_sodium = total("sodium_mg") / days
    avg_protein = total("protein_g") / days

    active_min = sum(
        float(a.get("minutes") or 0) * (1.5 if a.get("intensity") == "high" else 1.0)
        for a in activities
    )

    # Component scores (0-1). Calories gets full marks within +/-15% of target.
    ...
    act_s = _clamp(active_min / activity_target) if activity_target else 0
    consistency_s = _clamp(days_logged / consistency_target) if consistency_target else 0
    ...
    if avg_sugar > t["sugar_g_per_day"] * 1.3 and act_s < 0.5:
        signals.append({
            "type": "type2_diabetes",
            "level": "elevated",
            "why": f"Avg sugar {avg_sugar:.0f}g/day (target <= {t['sugar_g_per_day']}g) with only {active_min:.0f} active min this period.",
        })
    ...
    if act_s < 0.35:
        signals.append({
            ...
            "level": "elevated",
            "why": f"{active_min:.0f} of {activity_target:.0f} recommended active minutes for this period.",
        })
    if not signals:
        signals.append({
            "type": "none",
            "level": "healthy",
            "why": "No elevated risk patterns detected in this period. Keep it up!",
        })
    ...
    return {
        ...
        "signals": signals,
        "targets": {**t, "activity_minutes_for_period": round(activity_target), "period_days": period_days},
    }
